...
app.get('/', (req, res) => {
  res.send('Hello from firebase functions express API');
});
...
app.put('/recipes/:id', async (req, res) => {
  ...
  if (missingFields) {
    return res.status(400).json({
      error: true,
      message: `Resipe is not valid. Missing invalid fields: ${missingFields}`
    });
  }
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: `Success update recipe with id: ${id}` });
  } catch (error) {
...
app.delete('/recipes/:id', async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: 'Recipes successfully deleted' });
  } catch (error) {
    return res.status(400).json({ success: false, message: error.message });
  }
});
...
module.exports = app;
// firebase:authUser:AIzaSyCsdXWxIAXXLamI6iSNR9oZfSktgwp8xMo
